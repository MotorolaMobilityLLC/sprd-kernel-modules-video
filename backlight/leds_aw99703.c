...
static bool aw99703_probed = false;
static bool aw99703_flag = false;
bool check_aw99703_probed(void)
{
	return aw99703_probed;
}
EXPORT_SYMBOL(check_aw99703_probed);
...
static int aw99703_read_chipid(struct aw99703_data *drvdata)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		...
		if (ret < 0) {
			pr_err("%s: failed to read reg AW99703_REG_ID: %d, try num: %d\n",
				__func__, ret, cnt);
			cnt++;
...
static int aw99703_remove(struct i2c_client *client)
{
	...
	pr_info("%s enter! driver version \n", __func__);
	kfree(drvdata);
	return 0;
}
...
static int aw99703_suspend(struct device *dev)
{
	...
	pr_info("%s.\n", __func__);
	aw99703_hwen_pin_ctrl(drvdata, 0);
	aw99703_set_work_mode(drvdata, 0);
	return 0;
}

int aw99703_sleepin(void)
{
	struct aw99703_data *drvdata = g_aw99703_data;
	pr_info("%s start.\n", __func__);
	aw99703_set_work_mode(drvdata, 0);
	aw99703_hwen_pin_ctrl(drvdata, 0);
	pr_info("%s end.\n", __func__);
...
int aw99703_sleepout(void)
{
	...
	if(aw99703_flag) {
		...
		aw99703_pwm_mode_enable(drvdata);
		pr_info("%s ic type: aw99703.\n", __func__);
#else
		pr_info("%s ic type111: aw99703.\n", __func__);
		aw99703_backlight_init(drvdata);
#endif
	} else {
		pr_info("%s ic type: ktd3136.\n", __func__);
		aw99703_i2c_write(drvdata->client, 0x02, 0xF7);
		aw99703_i2c_write(drvdata->client, 0x06, 0x2F);
		aw99703_i2c_write(drvdata->client, 0x08, 0x13);
	}
	pr_info("%s end.\n", __func__);
	return 0;
}
EXPORT_SYMBOL(aw99703_sleepout);
